...
| Rust X11 client | `cargo run --offline -q -p sophia-cli -- x-authority-x11rb-smoke` | client-compatible setup, root/visual facts, atom/property/window flow | `client` | `proven`; setup now assigns disjoint client XID ranges, but request-level ownership and simultaneous-client dispatch are required before a shared-desktop proof. |
