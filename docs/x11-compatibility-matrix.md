...
| GTK software dialog | `dbus-run-session -- cargo run --offline -q -p sophia-cli -- x-authority-zenity-smoke` | GTK negotiates the probe-backed XKB/XI2 subset and commits a nonzero `MIT-SHM` image | `engine` | `proven` for nonzero software pixels with `first_error=none`; interactive GTK input and broader XI2 remain open. |
