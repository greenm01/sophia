...
| --- | --- | --- | --- | --- |
| Setup parser and admission | `cargo test --offline -q -p sophia-x-authority --test x11_wire` plus `cargo test --offline -p sophia-cli --features atomic-scanout-live live_xauthority_file_is_owner_only_valid_and_removed_on_drop -- --exact` | byte order, bounded setup fields, setup success/failure encoding, MIT-MAGIC-COOKIE-1 gating/provenance, peer-credential denial, concurrent identity allocation, revocation on disconnect/error, and owner-only Xauthority removal | `wire` | `proven`; each live classic session publishes a fresh cookie and requires same-UID peers, while the live launcher can explicitly allocate a confined group. Independently credentialed groups and supervisor-triggered revocation remain before a general local session. |
