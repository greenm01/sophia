...
| Guarded two-xterm session | `tools/live_session_two_xterm_hardware_proof.sh` | primary input proof plus a supervised secondary xterm must produce at least two CPU layers before existing KMS evidence validation passes | `hardware` | `operator gate`; run from an exclusive DRM/KMS TTY and retain the resulting evidence before promotion. |
