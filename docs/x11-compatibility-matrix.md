...
| --- | --- | --- | --- | --- |
| Setup parser and admission | `cargo test --offline -q -p sophia-x-authority --test x11_wire` | byte order, bounded setup fields, setup success/failure encoding, configured MIT-MAGIC-COOKIE-1 acceptance/rejection, peer-credential admission denial, concurrent identity allocation, and revocation on disconnect/error | `wire` | `proven`; the live classic session requires same-UID peers. Xauthority-file management, cookie rotation, and confined launch policy remain before a general local session. |
