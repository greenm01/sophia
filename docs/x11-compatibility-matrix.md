...
| --- | --- | --- | --- | --- |
| Setup parser and admission | `cargo test --offline -q -p sophia-x-authority --test x11_wire` plus `cargo test --offline -p sophia-cli --features atomic-scanout-live live_xauthority_file_is_owner_only_valid_and_removed_on_drop -- --exact` | byte order, bounded setup fields, setup success/failure encoding, MIT-MAGIC-COOKIE-1 gating/provenance, peer-credential denial, concurrent identity allocation, revocation on disconnect/error, and owner-only Xauthority removal | `wire` | `proven`; each live classic session publishes a fresh cookie and also requires same-UID peers. Confined launch policy remains before a general local session. |
...
| Rust X11 client | `cargo run --offline -q -p sophia-cli -- x-authority-x11rb-smoke` | client-compatible setup, root/visual facts, atom/property/window flow | `client` | `proven`; setup assigns disjoint client XID ranges, resource creation validates them, concurrent classic clients may use existing same-namespace resources, and policy-assigned confined clients receive `BadAccess` for cross-namespace map. The remaining confinement matrix is open. |
