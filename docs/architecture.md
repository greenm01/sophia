...
flags, native page-flip polling, callback intake, retirement, retire-time
resource destroy, and the evidence schema version collapse to reduced fields
only. A non-ready target fails the smoke evidence before a successful export or
submit can pass. The report deliberately omits card paths, file descriptors,
EGL displays, KMS object IDs, framebuffer IDs, and GEM handles.
