pub(crate) fn print(verbose: bool) {
    ...
    println!(
        "commands: sophia-live-session [--display=:77] [--terminal=xterm] [--input-devices=/dev/input/eventN,...] [--native-scanout] [--max-runtime-ms=N] [--max-ticks=N] [--inject-text=lowercase|--expect-physical-text=lowercase] [--proof]"
    );
    #[cfg(feature = "atomic-scanout-live")]
