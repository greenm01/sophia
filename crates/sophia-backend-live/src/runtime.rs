    ...
    P: NonBlockingInputPoller,
{
        ...
        R: LibdrmNativePageFlipReader,
    {
        ...
        let report = self
            .run_tick_with_rendered_primary_plane_scanout_and_native_page_flip_events_with(
                ...
                poller,
                sender,
                readiness.page_flip.max_read,
                readiness.page_flip.max_emit,
            )?;
        ...
        R: LibdrmNativePageFlipReader,
    {
        self.run_session_loop_tick_with_rendered_primary_plane_scanout_and_native_page_flip_events_with(
            input, readiness, device, exporter, reader, poller, sender,
        )
    }
}
...
pub struct LiveBackendSessionLoopReadiness {
    ...
    pub page_flip: LiveBackendSessionLoopPageFlipBudget,
}

#[cfg(all(feature = "libdrm-events", feature = "libinput-events"))]
impl LiveBackendSessionLoopReadiness {
    pub const fn new(input_ready: bool, page_flip: LiveBackendSessionLoopPageFlipBudget) -> Self {
        Self {
            input_ready,
            page_flip,
        }
    ...
    pub const fn idle(page_flip: LiveBackendSessionLoopPageFlipBudget) -> Self {
        Self::new(false, page_flip)
    }

    pub const fn input_ready(page_flip: LiveBackendSessionLoopPageFlipBudget) -> Self {
        Self::new(true, page_flip)
    }
}
