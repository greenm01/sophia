...
if [[ "$RUNS" != 3 ]]; then
    echo "DMA-BUF promotion requires exactly three independent Kitty runs." >&2
    exit 1
fi
if [[ ! -t 0 ]]; then
...
echo "[preflight] Running the controlled three-frame DMA-BUF first-frame proof."
...
echo "[0/3] Running the controlled 300-frame DMA-BUF lifecycle proof."
...
for run in 1 2 3; do
    ...
    rm -f "$evidence"
    echo "[$run/3] Run the guarded Kitty DMA-BUF proof and exit Kitty normally."
    SOPHIA_WAYLAND_KITTY_EVIDENCE="$evidence" \
        tools/wayland_kitty_hardware_proof.sh
done

echo "DMA-BUF promotion gate passed. Evidence: $EVIDENCE_DIR"
